# Global config
class Config:

    USERDATA_DIR = "userdata" # base directory for all userdata

    SESSION_FILE    = "{}/session.json".format(USERDATA_DIR) # session file
    CACHE_FILE      = "{}/cache.txt"   .format(USERDATA_DIR) # cache file
    QRCODE_FILE     = "{}/shaire.png"  .format(USERDATA_DIR) # shairing image file

    SIGNUP_TEXT     = "app/data/signuptext.txt" # text file for help message
    LICENCE         = "LICENCE" # licence file
    TERMS           = "app/data/terms.txt" # terms file
     
   # default data for the cahe
    DEFAULT_CACHE   = {
        "colorindex": dict() # depricated line
    }
    
    # default data for the session
    DEFAULT_SESSION = {
        "active":False, # indicates if to attempt login
        "friends":{ # list of friends
            # deafault data "Pending..." encoded in public key
            "empty"            : "-----BEGIN PGP PUBLIC KEY BLOCK-----\n\\n=eXjJ\n-----END PGP PUBLIC KEY BLOCK-----\n"
        }
    }

    # The app pretty name
    APPNAMELINK = "Kryptos"

    # Theme colors for the app

    SUPERBRIGHT       = "#000000"
    DARK              = "#121212"
    LIGHTBACKGROUND   = "#2E2E2E"
    BACKGROUND        = "#3B3B3B"
    BRIGHT            = "#787878"
    BUTTONBACKGROUND  = "#aaaaaa"
    SUPERDARK         = "#ffffff"

    ERROR             = "#aa0000"
    HEIGHLIGHT_COLOUR = "#064ca0"
    APP_COLOUR        = "#290f47"

    # font size and directory
    DATE_FONT_SIZE   = 12
    FONT = "app/fonts/LEMONMILK-Medium.ttf"

    # notifications display time before disapearing
    NOTIFICATION_DISPLAY_TIME = 1.5